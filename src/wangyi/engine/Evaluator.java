...
import wangyi.parser.AddExpr;
import wangyi.parser.Assignment;
import wangyi.parser.Block;
import wangyi.parser.ConditionalExpr;
import wangyi.parser.Constant;
import wangyi.parser.Expression;
import wangyi.parser.ExpressionList;
import wangyi.parser.FunctionCall;
import wangyi.parser.IfStatement;
import wangyi.parser.Statement;
import wangyi.parser.StatementList;
import wangyi.parser.Variable;
import wangyi.parser.VariableDotConstant;
...
//		System.out.println(ce.getComparator());
	...
	private static Value evaluateFunctionCall(FunctionCall functionCall, Player player, Player enemy, Event event, int index) 
	{
		...
		else if(functionCall.getName().equals("lifeset"))
		{
			Value target=evaluateExpression(functionCall.getExpressionList().getExpression(),player,enemy,event, index);
			Value count=evaluateExpression(functionCall.getExpressionList().getRemainingList().getExpression(),player,enemy,event, index);
			
			player.lifeSet(target, count.getIntValue());
