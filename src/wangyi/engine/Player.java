...
import wangyi.dao.GameCardDao;
import wangyi.dao.GameDao;
import wangyi.dao.GameListenerDao;
import wangyi.dao.HeroDao;
import wangyi.dao.HeroMinionDao;
import wangyi.dao.MinionDao;
import wangyi.dao.WeaponDao;
import wangyi.data.Card;
import wangyi.data.Event;
import wangyi.data.Game;
import wangyi.data.GameCard;
import wangyi.data.GameListener;
import wangyi.data.Hero;
import wangyi.data.HeroMinion;
import wangyi.data.Minion;
import wangyi.data.Target;
import wangyi.data.Weapon;
import wangyi.parser.ConditionalExpr;
...
//	}
	...
	public void makeGameCardTarget()
	{
		...
		for(GameCard gc: this.handGameCards)
		{
			...
			else if(card.getTargetCode().startsWith("doublechoice")) // todo
				gc.setNeedTarget(false);
	...
	public void userHeroSkill(String tid)
	{
		if(tid.equals(" "))
			...
			this.currentTarget=EngineHelper.makeValue(tid);
	...
	public void useGameCard(String cid, String tid, int index)
	{
		...
		this.currentSource=EngineHelper.makeValue(cid);
	...
	public void useCard(Card card, int index)
	{
		if(card.getGroup().equals("英雄级") || card.getGroup().equals("附属级"))
		{
			...
			{
				this.currentSource=EngineHelper.makeValue(createMinion(card, index).getId());
				Evaluator.evaluateCard(card, index, this, this.enemy);
			...
			{
				createWeapon(card);
				this.currentSource=EngineHelper.makeValue(weapon.getId());
				Evaluator.evaluateCard(card, index, this, this.enemy);
			}
		}
		else
	...
	public void minusAttackCount(String cid)
	{
		...
		if(heroMinion.getId().equals(cid))
		{
			...
			if(this.weapon.getBlood()==0)
			{
				...
				event.setEventSource(EngineHelper.makeValue(weapon.getId()));
	...
	public void createWeapon(Card card)
	{
		...
		weapon.setState("");
		weapon.setInnerState(EngineHelper.parseState(weapon.getState()));
...
//	}
		...
		for(String id: target.getSet())
		{
			...
			if(id.equals(enemy.weapon.getId()))
			{
				...
				if(enemy.weapon.getBlood()<=0)
				{
					...
					Event event=new Event();
					event.setName("weaponlostevent");
					event.setEventSource(EngineHelper.makeValue(enemy.weapon.getId()));
	...
	public void updateMinions()
	{
		...
		for(Minion minion: copy)
		{
			...
			if(blood<=0)
			{
				...
				event.setEventSource(EngineHelper.makeValue(minion.getId()));
	...
	public void lifeSet(Value target, int count)
	{
		for(String id: target.getSet())
		{
			Minion minion=findMinionById(id);
			...
			if(minion!=null)
			{
				minion.setBlood(count);
				
			}
									
			if(id.equals(this.weapon.getId()))
			{
				weapon.setBlood(count);
...
//	}
	...
	public void realDamage(Value source, Value target, int count)
	{
		for(String id: target.getSet())
		{
			...
			if(minion!=null)
			{
				...
				event.setEventTarget(EngineHelper.makeValue(id));
	...
	public boolean setContext(String id)
	{
		...
		Value value=EngineHelper.makeValue(id);
