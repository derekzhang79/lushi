...
Card.prototype.renderState = function () {
    var me = this;
    if (me.d.usable) {
        setTimeout(
        	function () {
...
Card.prototype.use = function (index) {
    ...
    // 判断是否能量足够
    var power = this.pool.field.d.player.power;
    if (this.d2.cost > power) {
        r.error("魔法不足！");
        this.cancel();
        return;
    }
...
Card.prototype.targets = function (arr) {
    ...
    if(this.d.targets.length>0)
    	this.d.needTarget=true;
    else
    	this.d.needTarget=false;
...
Card.prototype.usable = function (value) {
	...
	//alert(this.name+"'s usable:"+ value );
	this.d.usable=value;
